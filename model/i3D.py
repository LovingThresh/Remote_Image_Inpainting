...
class Unit3D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_shape=(1, 1, 1), stride=(1, 1, 1), padding=0, activation_fn=F.relu or None,
                 norm="BN" or None, use_bias=False, name="Unit3D"):
        ...
        self.conv = nn.Conv3d(self.in_channels, self.out_channels, self.kernel_shape, self.stride,
                              self.padding, bias=self.use_bias)

        if norm == "BN":
            self.norm_layer = nn.BatchNorm3d(out_channels, eps=0.001, momentum=0.01)
        elif norm == 'IN':
...
class InceptionI3d(nn.Module):
    ...
    def __init__(self, num_classes=400, spatial_squeeze=True,
        ...
        self.end_points[end_point] = Unit3D(in_channels=in_channels, out_channels=64, kernel_shape=[7, 7, 7],
        ...
        self.end_points[end_point] = Unit3D(in_channels=64, out_channels=64, kernel_shape=[1, 1, 1], padding=0,
                                            name=name + end_point)
        if self._final_endpoint == end_point:
            return

        end_point = 'Conv3d_2c_3x3'
        self.end_points[end_point] = Unit3D(in_channels=64, out_channels=192, kernel_shape=[3, 3, 3], padding=1,
                                            name=name + end_point)
        if self._final_endpoint == end_point:
            return
