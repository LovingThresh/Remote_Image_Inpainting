...
class VideoInpaintingModel(BaseModel):
    def __init__(self, opts, nc_in=5, nc_out=3, d_s_args=None, d_t_args=None):
        super().__init__()
        if d_t_args is None:
            d_t_args = {}
        if d_s_args is None:
...
d_t_args = {
    ...
    "conv_type": "vanilla",
    "conv_by": "2dtsm"}
model = VideoInpaintingModel(opti, d_s_args=d_s_args, d_t_args=d_t_args)
model = model.cuda()
a = torch.ones((6, 4, 3, 128, 128), dtype=torch.float)
b = torch.ones((6, 4, 1, 128, 128), dtype=torch.uint8)
for i in range(100):
